...
class Library_request {
    ...
    public function get($key, $filter_type = false){
        
        if( isset($_GET[$key]) ) {
            
            if( $filter_type != false)
               return filter_input(INPUT_GET, $key, $filter_type);
            else
                return $_GET[$key];
        }
        else {
    ...
    public function post($key, $filter_type = false){
        
        if( isset($_POST[$key]) ) {
            
            if( $filter_type != false)
               return filter_input(INPUT_POST, $key, $filter_type);
            else
                return $_POST[$key];
        }
        else {
    ...
    public function cookie($key, $filter_type = false){
        
        if( isset($_COOKIE[$key]) ) {
            
            if( $filter_type != false)
               return filter_input(INPUT_COOKIE, $key, $filter_type);
            else
                return $_COOKIE[$key];
        }
        else {
